...
export class StorageService {
  ...
  async uploadItem(
    ...
    try {
      ...
      const storageRef = this.firebase.storage.bucket(
        'lnco-artifacts.appspot.com',
      );
      ...
      return {
        url:
          `https://storage.googleapis.com/lnco-artifacts.appspot.com/` +
          `${_ref ? `images/${_ref}/${guid}` : `images/${guid}`}`,
        fileRef: `https://storage.googleapis.com/lnco-artifacts.appspot.com/${filePath}`,
        expiresBy: 0,
      };
    } catch (error) {
